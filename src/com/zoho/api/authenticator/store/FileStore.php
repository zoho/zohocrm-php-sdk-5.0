...
class FileStore implements TokenStore
{
    ...
    private $headers = array(Constants::ID, Constants::USER_NAME, Constants::CLIENT_ID, Constants::CLIENT_SECRET, Constants::REFRESH_TOKEN, Constants::ACCESS_TOKEN, Constants::GRANT_TOKEN, Constants::EXPIRY_TIME, Constants::REDIRECT_URL);
    ...
    public function getTokens()
    {
        ...
        try
        {
            ...
            for ($index = 1; $index < sizeof($csvReader); $index++)
            {
                ...
                $redirectURL = $this->getData($nextRecord[8]);

                $token = (new OAuthBuilder())->clientId($nextRecord[2])->clientSecret($nextRecord[3])->refreshToken($nextRecord[4])->grantToken($grantToken)->build();
