...
use com\zoho\crm\api\massdeletecvid\RecordIdBodyWrapper;
...
class MassDeleteRecordId
{
    ...
    public static function massDeleteByRecordId(string $moduleAPIName)
    {
        $massDeleteCvidOperations = new MassDeleteCvidOperations($moduleAPIName);
        $bodyWrapper = new RecordIdBodyWrapper();
        $bodyWrapper->setIds([44024800326053, 440248001304007]);
        $territory = new Territory();
        $territory->setId("0");
        $territory->setIncludeChild(true);
        $bodyWrapper->setTerritory($territory);
        $response = $massDeleteCvidOperations->massDeleteByRecordId($bodyWrapper);
        if ($response != null) {
...
MassDeleteRecordId::massDeleteByRecordId($moduleAPIName);
